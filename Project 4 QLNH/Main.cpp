...
int LoadBuyGoods()
{
	...
	while (true)
	{
		if (GetAsyncKeyState(VK_UP))
		{
			...
			{
				if (_y != ymax + 1)
					WriteTextAndColorAndWidth(_x - 2, _y - 1, "BUY ", 9, 13);
				_y -= 2;
				gotoxy(_x, _y);
				WriteTextAndColorAndWidth(_x - 2, _y - 1, "BUY ", 132, 13);
				gotoxy(_x, _y);
				Sleep(250);
				SelectedBuy -= 1;
			}
			else if (_y >= ymax - 1 && _x == 45)
			{
				...
				WriteTextAndColorAndWidth(_x + 91, _y - 5, "BUY ", 132, 13);
				_x += 93;
				_y -= 4;
				gotoxy(_x, _y);
		...
		if (GetAsyncKeyState(VK_DOWN))
		{
			if (_y < ymax - 1)
			{
				if (_y != 19) WriteTextAndColorAndWidth(_x - 2, _y - 1, "BUY ", 9, 13);
				_y += 2;
				WriteTextAndColorAndWidth(_x - 2, _y - 1, "BUY ", 132, 13);
				gotoxy(_x, _y);
				Sleep(250);
			...
			{
				if (_y != 19) WriteTextAndColorAndWidth(_x + 93, _y + 4, "BUY ", 9, 13);
				_x -= 93;
				_y += 4;
				gotoxy(_x, _y);
